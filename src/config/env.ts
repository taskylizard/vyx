...
export function loadKanikouEnv(environment: NodeJS.ProcessEnv = process.env) {
  ...
  const raw = createEnv(kanikouEnvDefinition, { env: environment })
  ...
  const observability = match(raw.NODE_ENV)
    ...
    .with('production', () => {
      ...
      if (logsDataset === tracesDataset) {
        throw new Error('AXIOM_LOGS_DATASET and AXIOM_TRACES_DATASET must be different datasets.')
      }
