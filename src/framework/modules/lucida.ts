...
export class LucidaModule {
  ...
  public async download(ctx: Context, url: string) {
    ...
    const res = await this.retry(() => this.lucida.getByUrl(url))
    ...
    return match(res)
      ...
      .with({ type: 'album' }, async (album) => {
        ...
        for (const _track of album.tracks) {
          ...
          const { stream, extension } = await this.getStream(
  ...
  private async downloadTrack(track: TrackGetByUrlResponse) {
    const { stream, extension } = await this.getStream(track)
  ...
  private async retry<T>(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (error.message === 'ARL expired' && attempt < maxAttempts) {
          this.logger.info(
            `ARL expired. Attempting to reinitialize (Attempt ${attempt}/${maxAttempts})`
          )
          await this.init()
