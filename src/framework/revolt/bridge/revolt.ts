...
import type { Message, Client as RevoltClient } from 'revolt.js';
import type {
  AttachmentType,
  Mapping,
  ReplyObject,
...
async function formatMessage(revolt: RevoltClient, message: Message) {
  ...
  if (pings && message.mentionIds) {
    for (const ping of pings) {
      ...
      if (matched !== null) {
        ...
        if (id) {
          const match = message.mentionIds.find((member) => member === id);

          if (match) {
            content = content.replace(ping, `@${match}`);
          }
        }
      }
    }
  ...
  if (message.attachments !== undefined) {
    message.attachments.forEach((attachment) => {
      messageString += `${revolt.proxyFile(attachment.url)}\n`;
    });
  }
...
export async function handleRevoltMessage(
  ...
  try {
    const channel = discord.getChannel(target.discord);
