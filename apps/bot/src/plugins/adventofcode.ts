...
function formatTable(rows: string[][]) {
  ...
  return (
    ...
    rows
      .map((row) =>
        row.map((a, b) => a.padStart(highestLengths[b] ?? 0, ' ')).join('    ')
      )
      .join('\n')
  )
}
    ...
    async function postMessage(client: Client, channels: string[]) {
      ...
      const [lastStarTs, lastStarUser] = leaderboard.reduce<[number, string]>(
        ([lastTs, lastUser], user) => [
          ...
          user.last_star_ts > lastTs ? (user.name ?? 'Anon') : lastUser
        ],
        [0, 'Noone']
      )

      const digits = leaderboard[0]
        ? Math.floor(Math.log10(leaderboard[0].stars) + 1)
        : 1
