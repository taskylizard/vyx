...
export class AIModule {
  ...
  private aiLogger = logger.withTag('AIModule')
  ...
  public async chat(prompt: string, useTools = false) {
...
`,
  ...
  public async smugshroom(_input: ReducedMessage[]) {
    let input = null
    if (typeof input !== 'string') {
      input = _input.map((e) => textifyMessageForTaskModels(e)).join('\n')
    }
    this.aiLogger.info('Input:', input)
    const res = await fetch(this.client.env.SMUGSHROOM_API, {
      method: 'POST',
      headers: {
    ...
    this.aiLogger.info('Summary:', summary)
    return summary
  }
}